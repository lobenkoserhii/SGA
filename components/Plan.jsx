...
const ServiceCard = ({ service }) => (
  ...
  <h3 className="font-cormorant text-white text-3xl font-bold leading-[34px]">
    {service.title}
        ...
        <div className="font-outfit text-Gold text-sm mt-2 mb-4 border-b border-[#3A4553]">{service.method}</div>
        ...
        <div className="font-outfit text-WH text-sm mb-4 p-4 border-y border-[#3A4553]">{service.duration}</div>
        <ul className="font-outfit text-WH text-sm mb-4">
          {service.description.map((item, index) => (
            <li key={index} className={`border-b px-8 py-4 border-[#3A4553] ${index === service.description.length - 1 ? '' : 'mb-4'}`}>{item}</li>
          ))}
        </ul>
      </div>
    ...
    return (
      <div className="mx-auto max-w-[1440px] p-10">
        <div className='flex flex-col gap-5 justify-center items-center'>
          <div className="text-center text-[45px] font-bold font-cormorant leading-[51.57px]">Plan For Immigration</div>
